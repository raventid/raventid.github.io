...
Hello, my dear friend, today I invite you to join me in a fun journey on learning about SAT-solvers!
...
I will not refer to this cargo project much in the article, what you can do is to copy final code we will have into the main.rs file and run it. This way you will receive the same output I will show in this article, and you can also play around with different inputs!

Before we write some code, let's figure out what SAT-solver should do. SAT asks: given a Boolean formula over variables x_{1}, x_{2}, …, is there an assignment of =true= and =false= values that makes the whole formula =true=?
...
The code uses a variant of the standard DIMACS convention (https://web.archive.org/web/20190325181937/https://www.satcompetition.org/2009/format-benchmarks2009.html): a variable is a positive integer, and a literal is 3 (meaning "x_{3} is =true=") or -3 (meaning "x_{3} is =false=")

#+begin_src rust
type Literal = i32;
#+end_src

A Clause is a connection of 2 literals, do not be confused by a vec!, it is always just 2 elements (TODO: proof-check) so it is Literal_{1} \lor{} Literal_{2} \lor{} Literal_{3}:
...
A final element we need is a Formula - this is the whole expression we want to check Clause_{1} \land{} Clause_{2} \land{} Clause_{3}:
...
Let me illustrate this with an example: {{{f((x_{1} \lor{} x_{2}) \land{} (x_{3} \lor{} x_{4}) \land{} (\neg{}x_{1} \lor{} x_{5}))}}}:

Let's pick x_{1} and make it =true=: {{{f((true \lor{} x_{2}) \land{} (x_{3} \lor{} x_{4}) \land{} (\neg{}x_{1} \lor{} x_{5}))}}}
Now we can remove clause 1! Why? because inside the clause we always use \lor{}, and only one =true= is enough to convert whole clause into the =true=!

So, we are left with {{{f((x_{3} \lor{} x_{4}) \land{} (\neg{}x_{1} \lor{} x_{5}))}}}

We removed all the clauses with x_{1}, but we didn't touch \neg{}x_{1} clauses yet (because in DIMACS notation we searched for 1, but we didn't search for -1). We are doing it now in the =reduced= action.
...
Now the logic is this one, we already picked x_{1} as a =true= and simplified the whole formula based on the fact it is =true=. So we cannot just repick =false= easily. It means that \neg{}x_{1} is gonna be =false= in this clause and =false= cannot help us to convert clause to =true=, so we remove it (think through, why do we do this?).
...
If it would be empty - there would be no way for us to make it =true=, because it would mean that \neg{}x_{1} was the last literal there and it was converted to =false=! So in this case we return =None= and it means that we were wrong about our assumption that x_{1} should be =true=. Maybe we should try x_{2}?

One important thing to notice here is that our function does not differentiate between x_{1} and \neg{}x_{1}. What I mean by that is that this function idea is to set literal to =true=, and it could be x_{1} literal and \neg{}x_{1} literal. In case caller will ask us to set \neg{}x_{1} to =true=, we will use =-literal= to get x_{1}. So, =-literal= in the code above is not transforming x_{1} to \neg{}x_{1}, it could also transform \neg{}x_{1} to x_{1} (so it is not negation, but reversal).
...
If we think in the DIMACS encoding our unit literal is 1 for x_{1} and -1 for \neg{}x_{1}. As you remember - the whole goal we are trying to reach is to convert whole formula to =true=. So if we see {{{f((x_{1}))}}} we understand that it should be =true= to convert this clause to =true=. If we see {{{f((\neg{}x_{1}))}}} then x_{1} should be =false=, to convert clause to =true=!
